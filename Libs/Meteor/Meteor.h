...
typedef enum {
   ...
   SIGTYPE_COUNT = 0x00000029,   // Count of valid SigTypes
} SigTypes;
...
typedef enum {
   ...
   HT_PQ512 = 0x0000002D,   // Dimatix QClass PQ-512 head. New software design for Polaris head (HT_POLARIS)
   HT_QS256 = 0x0000002E,   // Dimatix QClass QS-256 head. New software design for Q-class (Sapphire) head (HT_QCLASS)
   ...
   HT_KY0360_TF = 0x00000031,   // Kyocera KJ4C-0360 (Technoferrari version) head
   HT_COUNT = 0x00000032,   // Count of HT_xxx values
} eHEADTYPE;

// Status requests types. 
...
#define BMPS_USBHIGHSPEED 0x00000040   // USB connected at high speed
...
#define BM_COUNTUP 0x00000020   // Ignore encoder direction
#define BM_DEJITTER 0x00000040   // Enable jitter filter
...
#define BM_RTOLCFG 0x00010000   // RTOL as defined in config file
#define BM_STR_UNDER 0x00020000   // Strict underrun detect
...
#define MAX_IMAGE_REF 0x00000020   // The image reference parameter sent to <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_WRITE_RAM_IMAGE" /> or <see cref="F:Ttp.Meteor.CtrlCmdIds.PCMD_WRITE_RAM_IMAGE_EX" />
                                   // must be between 0 and (MAX_IMAGE_REF-1) inclusive
...
#define MAX_INTERLACE 12   // Max interlace levels
...
#define IMG_BUF_UNSET 0xFFFFFFFF   // Returned by PiAllocateImageBuffer if image buffer allocation fails
#define DATAPATH_FEATURE_FIFO 1   // FIFO datapath available
#define DATAPATH_FEATURE_PRELOAD 2   // PRELOAD datapath available
#define DATAPATH_FEATURE_CFIMAGE 4   // PCMD_CFIMAGE available
#define DATAPATH_FEATURE_SCREENPRO 8   // Screening of contone images available (including MetFlat)
#define DATAPATH_FEATURE_RIP 16   // Raster Image Processing of pdf files, RGB TIFF files setc. available
...
typedef enum {
   ...
   PCMD_PRINT_RAM_IMAGE = 0x7A5535BA,   // Print Image Stored in PCC RAM
                                        ...
                                        // Cmd[2] = Image Reference Number
                                        // Cmd[3] = X Start (pixels)
                                        // Cmd[4] = Width (pixels); used for image positioning within the document when printing is left-to-right.  Should normally match the width in PCMD_WRITE_RAM_IMAGE.
                                        // Cmd[5] = [Optional] X Offset (pixels) within the image for circular printing
   PCMD_WRITE_RAM_IMAGE = 0x7A5535BB,   // Write Image to PCC RAM
                                        // Used to instruct PCC to write image to RAM, which can then be printed with PCMD_PRINT_RAM_IMAGE
                                        // Cmd[0] = PCMD_WRITE_RAM_IMAGE
                                        // Cmd[1] = Command DWORD count (4 + image DWORDs)
                                        // Cmd[2] = Image Reference Number, 0 to <see cref="F:Ttp.Meteor.MeteorConsts.MAX_IMAGE_REF" />-1 (this will be used to identify this image for later PCMD_PRINT_RAM_IMAGE commands)
   ...
   PCMD_IMAGE_BUFFER = 0x7A5535BC,   // Send an image from an image buffer previously allocated and filled by PiAllocateTiffImageBuffer, or allocated by 
                                     // PiAllocateImageBuffer and filled with PiFillImageBuffer, etc.
                                     // Note that PiFillImageBuffer is avaliable only when the calling application is also hosting the Meteor Print Engine
   ...
   PCMD_WRITE_RAM_IMAGE_EX = 0x7A5535BE,   // Write Image to PCC RAM
                                           // Used to instruct PCC to write image to RAM, which can then be printed with PCMD_PRINT_RAM_IMAGE
                                           // Similar to PCMD_WRITE_RAM_IMAGE, but also allows the amount of memory reserved for the reference to be defined
                                           // Cmd[0] = PCMD_WRITE_RAM_IMAGE_EX
                                           // Cmd[1] = Command DWORD count (6 + image DWORDs)
                                           // Cmd[2] = Header DWORD count (5)
                                           // Cmd[3] = Image Reference Number, 0 to <see cref="F:Ttp.Meteor.MeteorConsts.MAX_IMAGE_REF" />-1 (this will be used to identify this image for later PCMD_PRINT_RAM_IMAGE commands)
                                           // Cmd[4] = [Bits7:0] Plane (1-<see cref="F:Ttp.Meteor.MeteorConsts.MAX_PLANES">MAX_PLANES</see>); Bit31 set for circular printing; Bit30 set if more images (of the same width) for the same reference will follow; Bit29 sets scan direction (clear=FWD, set=REV)
                                           // Cmd[5] = YTop (pixels)
                                           // Cmd[6] = Width (pixels)
   ...
   PCMD_CF_TRANSLATEIMAGE = 0x7A5535BF,   // Prepare an image for writing to Flash memory (PCC8 Compact Flash or PCCE UHS SD Card) using the translator
                                          ...
                                          // An image can be loaded into an image buffer via PiAllocateImageBuffer and PiFillImageBuffer; PiAllocateTiffImageBuffer; or 
                                          ...
                                          // PiAllocateImageBuffer and PiFillImageBuffer are avaliable only when the calling application is also hosting the 
                                          // Meteor Print Engine
                                          ...
                                          // Cmd[2] = Plane (1-<see cref="F:Ttp.Meteor.MeteorConsts.MAX_PLANES">MAX_PLANES</see>).  Top bit set to free image buffer after use.  
                                          // Send Plane == 0 to free buffer without using its contents. 
                                          // Cmd[3] = YTop (pixels)
                                          // Cmd[4] = Width (pixels).  If an image buffer is used, and the buffer contents were loaded from a file 
                                          // (PiAllocateTiffImageBuffer etc.), then Width should be set to zero (which means use the file's width).
                                          // Cmd[5] = Image buffer ID; -1 to fill all head Flash memory slots with zero [Ignored if the image data is sent as part of the 
                                          // command buffer]
   ...
   PCMD_RIPIMAGE = 0x7A5535C5,   // Rip-and-print image command id
                                 // RIPs the image and prints it either as individual FIFO documents or as a continuous FIFO document if the job is running in 
                                 // continuous mode (which can be set in the config file, or by calling PCMD_SET_CONTINUOUS_PRINT)
                                 ...
                                 // Cmd[6] = Plane (P) for channel 1 of the ripped image; P is indexed from 1; channel N of time image uses plane P + (N-1)
                                 ...
                                 // Cmd[12] = Control flags: [BITS1:0] rotate in multiples of 90 degrees: 0 = 0; 1 = -90; 2 = 180; 3 = -90; 
                                 ...
                                 // Cmd[13] = Doc Id for the first page in the document; incremented for each page when printing a page per Product Detect
...
typedef enum {
   ...
   CCP_BITS_PER_PIXEL = 0x00000026,   // Overrides default bits per pixel for current head type, if supported
   ...
   CCP_TRANSLATOR_MODE = 0x0000004E,   // BIT0: set for optimised image transpose translator.  BIT1: set for parallel planes translation.  BIT2: set for SIMD image transpose.
...
typedef enum {
   ...
   CPEX_X_Offset = 0x00000110,   // Nozzle Rows X offsets (centipixels)
   ...
   CPEX_TargetTemp = 0x00000116,   // Target temperature
                                   // Head Types: HT_KM300, HT_KM_M600, HT_KM1024I, HT_KM1800I, HT_SPT_1024GS, HT_SPT_RC1536
   ...
   CPEX_WF_FileIdx = 0x0000011C,   // Waveform file index.  Note: indexing starts from 1.  Negative values can be used to select built-in waveforms (HT_4CF1 only).
                                   // Head Types: HT_4CF1, HT_KM130, HT_KM_M600, HT_KM1024I, HT_KM1800I, HT_SAMBA, HT_GH2220, HT_SPT_1024GS, HT_SPT_RC1536
   ...
   CPEX_WF_VScaleCoeff = 0x00000122,   // Waveform voltage scale coefficient (0.5 to 1.5)
                                       // Head Types: HT_SAMBA, HT_GMA33, HT_GMA99, HT_SG600; HT_KM130, HT_KM_M600, HT_KM1024I (scales VH2 only), HT_KM1800I (scales VH2 only), HT_KM1024 (scales VH2 only); HT_Xaar1201
...
typedef enum {
   ...
   RVAL_EXCEPTION = 0x00000020,   // An exception occured when attempting to start the PrintEngine; see Windows debug output for details
...
typedef struct {
   ...
   int32   Yinterlace;   // Number of heads interlaced
   ...
   int32   bmMeteorStatus;   // Additional meteor status bits BMMS_xxx
   ...
   int32   MixedModePreloadDocsQueuedLane2;   // Number of preload documents documents queued for print lane 2 in mixed mode
   ...
   int32   TracerCount;   // Increments (or resets) once a PCMD_TRACER command has been processed by every PCC in the system.  Ethernet hardware only.
} TAppStatus;
...
typedef struct {
   ...
   int32   Temperature1;   // HDC temperature 1 (0.1C):
                           // Head temp HT_LEOPARD, HT_HSS, HT_XJ500, HT_K150, HT_K300, HT_K600, HT_K1200, HT_KJ4, HT_SPT_254GS, HT_SPT_510BIN, HT_LEXMARK, HT_SPT_508GS, HT_STARFIRE, HT_GMA33, HT_GMA99, HT_SAMBA, HT_RG5, HT_SPT_RC512;
   ...
   int32   Temperature2;   // HDC temperature 2 (0.1C):
                           ...
                           // Head amp temp for HT_GMA33, HT_GMA99, HT_SPT_RC512;
                           // Head amp temp A for HT_SAMBA, HT_SPT_508GS;
   ...
   int32   Temperature3;   // HDC temperature 3 (0.1C):
                           ...
                           // Head amp temp B for HT_SAMBA, HT_SPT_508GS;
   ...
   int32   Voltages[32];   // HDC voltages. Values interpretation depends on the HDC type. This is defined in section 8.10 of the software user interface manual
...
typedef struct {
   ...
   int32   Reserved[32];   // Reserved for future use
...
typedef struct {
   ...
   int32   CropXStart;   // [In] X position to start cropping the image
   int32   CropYStart;   // [In] Y position to start cropping the image
   int32   CropWidth;   // [In/Out] Width of the cropped image
   int32   CropHeight;   // [In/Out] Height of the cropped image
   ...
   int32   BufferWidth;   // [In] Buffer width if the buffer is created larger than the (cropped) image - 
                          // only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE or IMG_LOAD_X_REPEAT is set
   int32   BufferHeight;   // [In] Buffer height if the buffer is created larger than the (cropped) image - 
                           // only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set
   int32   BufferXPos;   // [In] X position within the buffer - only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set
   int32   BufferYPos;   // [In] Y position within the buffer - only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set
   int32   ImageXRepeat;   // [In] Repeat image in X after loading - only used if IMG_LOAD_X_REPEAT is set
   int32   ImageXRepeatGap;   // [In] Gap (pixels) between repeated X images - 
                              // only used if <see cref="F:Ttp.Meteor.ImageLoadDetails.ImageXRepeat" /> is more than 1 and IMG_LOAD_X_REPEAT is set
...
#define RipModeAdHoc 0xFFFFFFFF   // Create an ad-hoc RIP mode from the details in the <see cref="T:Ttp.Meteor.RipDetails" /> structure (rather than using a Meteor config file mode)
#define RipModeRipConfig 0xFFFFFFFE   // Use a pre-existing RIP configuration (in the RIP's TestConfig directory) rather than using a Meteor mode.
#define RipErrorNoLicence 0x00000001   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: RIP licence is not available or has expired
#define RipErrorNoLdk 0x00000002   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: The RIP licence manager is not running
#define RipErrorMissingFont 0x00000004   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: RIP processing failed due to a missing font
#define RipWarnMissingFont 0x00000008   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: A required font is missing and was substituted during RIP processing.  
                                        // The printed output may contain incorrect text.
#define RipNotComplete 0x00000010   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: RIP processing failed to complete
#define RipRefusedData 0x00000020   // Bit set in <see cref="F:Ttp.Meteor.RipDetails.RipOutputFlags" />: The RIP refused the data for the job
...
typedef struct {
   ...
   int32   ImgBufferIndexes[32];   // [OUT] filled in with the Meteor buffer indexes for the RIPPed channels.  IMG_BUF_UNSET (0xFFFFFFFF) for channels which don't exist.
   int32   RipOutputFlags;   // [OUT] Error or warning bits set on RIP completion; see <see cref="F:Ttp.Meteor.RipConsts.RipErrorNoLicence" /> etc.
   int32   RipMode;   // [IN] Index of the RIP Mode (1-N) definied in the Meteor configuration file
                      // Set to 0 to select the config file's default mode
                      // Set to RipModeAdHoc (-1) to create an ad-hoc mode from the details below
                      // Set to RipModeRipConfig (-2) to use a pre-existing RIP configuration in the TestConfig directory
   int32   Page;   // Which page to RIP for a multi-page PDF
   int32   CacheFlags;   // TODO
   int32   BitsPerPixel;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's bits per pixel
   int32   XDpi;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's X resolution
   int32   YDpi;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's Y resolution
   int32   Mirror;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's mirror
   int32   Orientation;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's rotation in multiples of 90 degrees
   double   XScale;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's X scaling; 1.0 for no scale
   double   YScale;   // [IN] If RipMode is -1, selects the ad-hoc RIP mode's Y scaling; 1.0 for no scale
   char   RipConfigName[256];   // [IN] If RipMode is -2, selects the pre-existing RIP config.  The config file must be in the HHR install's TestConfig directory.
} RipDetails;
